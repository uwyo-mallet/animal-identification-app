...
# test if tensorflow was imported
# modulename = "tensorflow"
# if modulename not in sys.modules:
#  print '{} has not been properly installed. Please install tensorflow, a version that is >1.8 and <2.0.'.format(modulename)
...
# import utils
from .utils import loss, get_optimizer, get_policy, AverageMeter, count_input_records
...
def do_train(sess, args):
    ...
    with tf.device("/cpu:0"):
        ...
        images_ph = tf.placeholder(
            tf.float32, shape=(None,) + tuple(args.processed_size), name="input"
        )
        ...
        # Weight Decay policy
        wd = get_policy(args.WD_policy, args.WD_details)

        # Learning rate decay policy (if needed)
        lr = get_policy(args.LR_policy, args.LR_details)

        # Create an optimizer that performs gradient descent.
        optimizer = get_optimizer(args.optimizer, lr)
        ...
        for epoch in range(start_epoch, start_epoch + args.num_epochs):
            ...
            print(
                "Epoch %d of %d ended. a checkpoint saved at %s"
            ...
            if args.run_validation:
                ...
                total_loss = AverageMeter()  # Measures cross entropy loss
                top1 = AverageMeter()  # Measures top-1 accuracy
                topn = AverageMeter()  # Measures top-n accuracy
                ...
                for step in range(args.num_val_batches):
                    ...
                    val_img, val_lbl = sess.run(
                        ...
                        feed_dict={
                            batch_size_tf: args.num_val_samples
                ...
                for i in range(0, val_inf.shape[0]):
                    out_file.write(
                        ...
                        % (
                            ...
                            val_inf[i],
                            val_loader.label_dict[val_lbl[i]],
        ...
        # inference
            ...
            for step in range(args.num_val_batches):
                ...
                topnguesses, topnconf = sess.run(
                    eval_ops,
                    feed_dict={images_ph: val_img, is_training_ph: False},
                    options=args.run_options,
                    run_metadata=args.run_metadata,
                )
...
def main():  # pylint: disable=unused-argument
    ...
    parser.add_argument(
        "command",
        action="store",
        help="Determines what to do: train, evaluate, or inference",
    )
    parser.add_argument(
        "--raw_size",
        nargs=3,
    ...
    parser.add_argument(
        ...
        type=int,
        action="store",
        help="The depth of ResNet or DenseNet architecture",
    )
    parser.add_argument(
        "--run_name",
        default="Run" + str(time.strftime("-%d-%m-%Y_%H-%M-%S")),
        action="store",
    ...
    parser.add_argument(
        ...
        type=int,
        action="store",
        help="Number of GPUs (Only for training)",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--delimiter", default=",", action="store", help="Delimiter of the input files"
    )
    parser.add_argument(
        "--retrain_from",
        default=None,
    ...
    parser.add_argument(
        "--LR_details",
        default="19, 30, 44, 53, 0.01, 0.005, 0.001, 0.0005, 0.0001",
        help="LR change details",
    )
    parser.add_argument(
        "--WD_details", default="30, 0.0005, 0.0", help="WD change details"
    )
    parser.add_argument(
        "--optimizer",
        default="momentum",
    ...
    parser.add_argument(
        ...
        type=int,
        action="store",
        help="Specify the top-N accuracy",
    )
    parser.add_argument(
        "--max_to_keep",
        default=5,
    ...
    if args.command.lower() == "train":
        # Assert input args
        assert (
            args.train_info is not None
        ), "No training dataset is provided, please provide an input file using --train_info option"

        # Counting number of training examples
        if args.num_batches == -1:
            args.num_samples, args.num_batches = count_input_records(
                args.train_info, args.batch_size
            )
        else:
            args.num_samples, _ = count_input_records(args.train_info, args.batch_size)

        # Counting number of validation examples
        if args.val_info is not None:
            args.num_val_samples, args.num_val_batches = count_input_records(
                args.val_info, args.batch_size
            )
        ...
        # Counting number of training examples
        assert (
            args.val_info is not None
        ), "No evaluation dataset is provided, please provide an input file using val_info option"
        args.num_val_samples, args.num_val_batches = count_input_records(
            args.val_info, args.batch_size
        )
        ...
        assert (
            args.val_info is not None
        ), "No inference dataset is provided, please provide an input file using --val_info option"
        args.num_val_samples, args.num_val_batches = count_input_records(
            args.val_info, args.batch_size
        )
    ...
    print("classify complete. predictions are stored at :", args.save_predictions)
