...
class model:
    ...
    def get_grads(self, device):
        ...
        with tf.device(device):
            ...
            top1acc = tf.reduce_mean(
                tf.cast(tf.nn.in_top_k(logits, self.labels, 1), tf.float32)
            )
            # Top-n accuracy
            topnacc = tf.reduce_mean(
                tf.cast(tf.nn.in_top_k(logits, self.labels, self.top_n), tf.float32)
            )
            ...
            last_grads = self.optimizer.compute_gradients(
                total_loss, var_list=tf.get_collection(SAVE_VARIABLES, scope="output")
            )
  ...
  """
    ...
    def multigpu_grads(self):
        ...
        return (
            self.average_gradients(tower_grads),
            self.average_gradients(tower_last_grads),
            batchnorm_updates,
            cross_entropy_mean,
            top1acc,
            topnacc,
  ...
  """
    ...
    def train_ops(self):
        ...
        if self.transfer_mode[0] == 2 or self.transfer_mode[0] == 0:
            ...
            train_op = tf.group(
                self.optimizer.apply_gradients(grads), batchnorm_updates_op
            )
        if self.transfer_mode[0] == 3:
            train_op = tf.cond(
                tf.less(
                    tf.get_collection(SAVE_VARIABLES, scope="epoch_number")[0],
                    self.transfer_mode[1],
                ),
                lambda: tf.group(
                    self.optimizer.apply_gradients(last_grads), *batchnorm_updates
                ),
                lambda: tf.group(
                    self.optimizer.apply_gradients(grads), *batchnorm_updates
                ),
            )
    ...
    def inference(self):
        if self.architecture.lower() == "alexnet":
            return alexnet(
                ...
                self.num_classes,
                self.wd,
                tf.where(self.is_training, 0.5, 1.0),
                self.is_training,
            )
            ...
            return densenet(
                self.inputs, self.depth, self.num_classes, self.wd, self.is_training
            )
            ...
            return googlenet(
                ...
                self.num_classes,
                self.wd,
                tf.where(self.is_training, 0.4, 1.0),
                self.is_training,
            )
    ...
    def save(self, sess, path, global_step):
        ...
        if self.saver is None:
            self.saver = tf.train.Saver(
                tf.get_collection(SAVE_VARIABLES), max_to_keep=self.max_to_keep
            )
    ...
    def load(self, sess, path):
        ...
        if self.pretrained_loader is None:
            if self.transfer_mode[0] == 0:
                self.pretrained_loader = tf.train.Saver(
                    tf.get_collection(SAVE_VARIABLES)
                )
            else:
