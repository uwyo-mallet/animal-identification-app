...
# ResNet Block
def resnetBlock(
    net, bottleneck, block_filters_internal, block_stride, wd=0.0, is_training=True
):
...
def getModel(
    ...
    with tf.variable_scope("scale1"):
        net = spatialConvolution(
            net, 7, 2, 64, weight_initializer=conv_weight_initializer, wd=wd
        )
        net = batchNormalization(net, is_training=is_training)
        net = tf.nn.relu(net)
    ...
    with tf.variable_scope("scale2"):
        net = maxPool(net, 3, 2)
        net = resnetStack(
            net, num_blocks[0], 1, 64, bottleneck, wd=wd, is_training=is_training
        )

    with tf.variable_scope("scale3"):
        net = resnetStack(
            net, num_blocks[1], 2, 128, bottleneck, wd=wd, is_training=is_training
        )

    with tf.variable_scope("scale4"):
        net = resnetStack(
            net, num_blocks[2], 2, 256, bottleneck, wd=wd, is_training=is_training
        )

    with tf.variable_scope("scale5"):
        net = resnetStack(
            net, num_blocks[3], 2, 512, bottleneck, wd=wd, is_training=is_training
        )
