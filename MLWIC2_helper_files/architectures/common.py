...
def _get_variable(
    name, shape, initializer, regularizer=None, dtype="float", trainable=True
):
...
def batchNormalization(x, is_training, decay=0.9, epsilon=0.001, inference_only=False):
    ...
    moving_mean = _get_variable(
        "moving_mean", params_shape, initializer=tf.zeros_initializer, trainable=False
    )
    ...
    return tf.cond(
        ...
        lambda: tf.nn.batch_normalization(
            x, moving_mean, moving_variance, beta, gamma, epsilon
        ),
    )
...
def fullyConnected(
    ...
    stddev = 1.0 / tf.sqrt(tf.cast(num_units_out, tf.float32))
    if weight_initializer is None:
        weight_initializer = tf.random_uniform_initializer(
            minval=-stddev, maxval=stddev, dtype=tf.float32
        )
...
def spatialConvolution(
    ...
    stddev = 1.0 / tf.sqrt(tf.cast(filters_out, tf.float32))
    if weight_initializer is None:
        weight_initializer = tf.random_uniform_initializer(
            minval=-stddev, maxval=stddev, dtype=tf.float32
        )
    ...
    weights = _get_variable(
        "weights", shape, weight_initializer, tf.contrib.layers.l2_regularizer(wd)
    )
...
def maxPool(x, ksize, stride):
    return tf.nn.max_pool(
        x, ksize=[1, ksize, ksize, 1], strides=[1, stride, stride, 1], padding="SAME"
    )
...
def avgPool(x, ksize, stride, padding="SAME"):
    return tf.nn.avg_pool(
        x, ksize=[1, ksize, ksize, 1], strides=[1, stride, stride, 1], padding=padding
    )
