...
def googlenet(net, num_output, wd, dropout_rate, is_training):
    ...
    with tf.variable_scope("inception3a"):
        net = inception(
            net, 64, [96, 128], [16, 32], 32, wd=wd, is_training=is_training
        )

    with tf.variable_scope("inception3b"):
        net = inception(
            net, 128, [128, 192], [32, 96], 64, wd=wd, is_training=is_training
        )
    ...
    with tf.variable_scope("inception4a"):
        net = inception(
            net, 192, [96, 208], [16, 48], 64, wd=wd, is_training=is_training
        )

    with tf.variable_scope("inception4b"):
        net = inception(
            net, 160, [112, 224], [24, 64], 64, wd=wd, is_training=is_training
        )

    with tf.variable_scope("inception4c"):
        net = inception(
            net, 128, [128, 256], [24, 64], 64, wd=wd, is_training=is_training
        )

    with tf.variable_scope("inception4d"):
        net = inception(
            net, 112, [144, 288], [32, 64], 64, wd=wd, is_training=is_training
        )

    with tf.variable_scope("inception4e"):
        net = inception(
            net, 256, [160, 320], [32, 128], 128, wd=wd, is_training=is_training
        )
    ...
    with tf.variable_scope("inception5b"):
        net = inception(
            net, 384, [192, 384], [48, 128], 128, wd=wd, is_training=is_training
        )
