...
class loader:
  ...
  """
    ...
    def _read_label_file(self):
        ...
        self.label_dict = {i: i for i in sorted(labels)}
        ...
        if not self.inference_only:
            ...
            if all([x.isdigit() for x in self.label_set]):
                ...
                labels = [
                    dict(zip(self.label_dict.values(), self.label_dict.keys()))[x]
                    for x in labels
                ]
  ...
  """
    ...
    def preprocess(self, filename):
        ...
        reshaped_image = tf.to_float(
            tf.image.decode_jpeg(file_content, channels=self.raw_size[2])
        )
        # Resize image to 256*256
        reshaped_image = tf.image.resize_images(
            reshaped_image, (self.raw_size[0], self.raw_size[1])
        )
    ...
    def load(self):
        ...
        if not self.inference_only:
            ...
            filename_queue = tf.train.slice_input_producer(
                [filenames, labels], shuffle=self.shuffle if self.is_training else False
            )
            ...
            print(
                "Filling queue with %d images before starting to train. "
            ...
            filename_queue = tf.train.slice_input_producer(
                [filenames], shuffle=self.shuffle if self.is_training else False
            )
            image_queue = filename_queue[0]
            reshaped_image = self.preprocess(image_queue)
            img_info = image_queue
  ...
  """
    ...
    def _test_preprocess(self, reshaped_image):
        ...
        resized_image = tf.image.resize_image_with_crop_or_pad(
            reshaped_image, self.processed_size[0], self.processed_size[1]
        )
