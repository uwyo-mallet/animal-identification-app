...
def count_input_records(filename, batch_size):
    ...
    return (
        num_samples,
        int(num_batches) if num_batches.is_integer() else int(num_batches) + 1,
    )
...
def get_policy(policy_type, details_str):
    ...
    if policy_type == "piecewise_linear":
        ...
        assert all(
            item.is_integer() for item in details[0 : int((length - 1) / 2)]
        ), "Invalid policy details"
        return tf.train.piecewise_constant(
            tf.get_collection(SAVE_VARIABLES, scope="epoch_number")[0],
            [int(x) for x in details[0 : int((length - 1) / 2)]],
            details[int((length - 1) / 2) :],
        )
    if policy_type == "exponential":
        ...
        return tf.train.exponential_decay(
            details[0],
            tf.get_collection(SAVE_VARIABLES, scope="global_step")[0],
            int(details[1]),
            details[2],
            staircase=False,
        )
